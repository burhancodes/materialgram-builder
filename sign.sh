...
if [ -z "$GPG_KEY_ID" ] || [ -z "$GPG_PASSPHRASE" ]; then
    echo "GPG_KEY_ID and GPG_PASSPHRASE environment variables must be set."
    exit 1
fi

# Sign the RPM package with loopback pinentry
echo "$GPG_PASSPHRASE" | gpg --batch --yes --pinentry-mode loopback --passphrase-fd 0 --detach-sign --default-key "$GPG_KEY_ID" "$RPM_PACKAGE"
...
if [ -f "$REPOMD_XML" ]; then
    echo "$GPG_PASSPHRASE" | gpg --batch --yes --pinentry-mode loopback --passphrase-fd 0 --detach-sign --default-key "$GPG_KEY_ID" "$REPOMD_XML"
